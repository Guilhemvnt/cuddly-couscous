...
int main() {
    ...
    ipHeader.ip_src.s_addr = inet_addr("192.168.1.48");
    ipHeader.ip_dst.s_addr = inet_addr("224.0.0.251"); // Set a valid destination IP

    // Fill in the TCP header
    struct tcphdr tcpHeader;
    memset(&tcpHeader, 0, sizeof(tcpHeader));
    tcpHeader.source = htons(12345);  // Source port
    tcpHeader.dest = htons(80);       // Destination port
    tcpHeader.seq = 0;
    tcpHeader.ack_seq = 0;
    ...
    // Send the packet
    ssize_t bytes_sent = sendto(sockfd, packet, ipHeader.ip_len, 0,
